...
def create_dataset(dataset_folder,dataset_name,val_size,gt,horizon,delim="\t",train=True,eval=False,perc_data=100,verbose=False):
        ...
        for i_dt, dt in enumerate(datasets_list):
            ...
            raw_data = pd.read_csv(os.path.join(full_dt_folder, dt), delimiter=delim, names=["frame", "ped", "x", "y"],usecols=[0,1,2,3],na_values="?")

            tot_num_peds = int(len(raw_data['ped'].unique()) * perc_data/100)
            which_peds = raw_data['ped'].unique()[:tot_num_peds]
            raw_data = raw_data[raw_data['ped'].isin(which_peds)]
